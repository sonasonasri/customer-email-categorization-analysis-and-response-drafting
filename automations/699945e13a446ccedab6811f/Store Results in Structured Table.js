...
const languages = getContext("languages") // NEW: Get detected language array
...
;(async () => {
  ...
  const table = emails.map((email, idx) => ({
    ...
    summary: summaries?.[0] || "",
    language: languages && languages[idx] ? languages[idx] : "Unknown", // NEW: Language field
  ...
  console.log(`📊 Fields: Email, Category, Sentiment, Urgency, Priority, Summary, Language, Timestamp`)
  console.log("=".repeat(50))
  ...
  if (table.length > 0) {
    ...
    console.log(`  Language: ${table[0].language}`) // NEW: Preview detected language
  }
})()
